...
class ShapingRainFrameworkOptionUpload extends ShapingRainFrameworkOption {
	 ...
	 * @since 1.9
	 *
	 * @see sr_upload_option_customizer_get_value()
	 */
	public function preLivePreview( $optionID, $optionType, $option ) {
		...
		$nonce = wp_create_nonce( 'sr_upload_option_nonce' );
		$size = ! empty( $this->settings['size'] ) ? $this->settings['size'] : 'thumbnail';

		?>
		wp.ajax.send( 'sr_upload_option_customizer_get_value', {
		    data: {
	...
	public function registerCustomizerControl( $wp_customize, $section, $priority = 1 ) {
		$wp_customize->add_control( new ShapingRainFrameworkOptionUploadControl( $wp_customize, $this->getID(), array(
 ...
 */
add_action( 'customize_register', 'registerShapingRainFrameworkOptionUploadControl', 1 );
function registerShapingRainFrameworkOptionUploadControl() {
	class ShapingRainFrameworkOptionUploadControl extends WP_Customize_Control {
		public $description;

		public function render_content() {
			ShapingRainFrameworkOptionUpload::createUploaderScript();
...
if ( ! function_exists( 'sr_upload_option_customizer_get_value' ) ) {

	add_action( 'wp_ajax_sr_upload_option_customizer_get_value', 'sr_upload_option_customizer_get_value' );
	 ...
	 */
	function sr_upload_option_customizer_get_value() {
		...
		if ( ! empty( $_POST['nonce'] ) && ! empty( $_POST['id'] ) && ! empty( $_POST['size'] ) ) {
			...
			if ( wp_verify_nonce( $nonce, 'sr_upload_option_nonce' ) ) {
