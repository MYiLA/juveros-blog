 ...
 * @since	1.4
 */
class ShapingRainFrameworkOptionFont extends ShapingRainFrameworkOption {
	...
	function __construct( $settings, $owner ) {
		...
		sr_add_action_once( 'admin_enqueue_scripts', array( $this, 'loadAdminScripts' ) );
		sr_add_action_once( 'customize_controls_enqueue_scripts', array( $this, 'loadAdminScripts' ) );
		sr_add_action_once( 'admin_head', array( __CLASS__, 'createFontScript' ) );
		sr_add_action_once( 'wp_enqueue_scripts', array( $this, 'enqueueGooglefonts' ) );
		sr_add_action_once( 'admin_enqueue_scripts', array( $this, 'enqueueGooglefonts' ) );
		add_filter( 'sr_generate_css_font_' . $this->getOptionNamespace(), array( $this, 'generateCSS' ), 10, 2 );

		// Customizer preview handling
		sr_add_action_once( 'sr_generate_customizer_preview_js', array( $this, 'generateCustomizerPreviewJS' ) );
		sr_add_filter_once( 'sr_generate_customizer_preview_css_' . $this->getOptionNamespace(), array( $this, 'generateCustomizerPreviewCSS' ) );
	 ...
	 * @return void
	 *
	 * @see ShapingRainFrameworkCustomizer->livePreviewMainScript()
	 */
	...
	public function getGoogleFontURLs() {
		...
		$subsets = apply_filters( 'sr_google_font_subsets_' . $this->getOptionNamespace(), array( 'latin', 'latin-ext' ) );
		...
		foreach ( $fontsToLoad as $fontName => $variants ) {
			...
			$fontUrl = apply_filters( 'sr_enqueue_google_webfont_' . $this->getOptionNamespace(), $fontUrl, $fontName );
	...
	/**
	 * Generates CSS for the font, this is used in ShapingRainFrameworkCSS
	 *
	 * @param	string               $css The CSS generated
	 * @param	ShapingRainFrameworkOption $option The current option being processed
	...
	public function generateCSS( $css, $option ) {
		...
		$cssVariables = '';
		$cssChecking = array( 'font_family', 'color', 'font_size', 'font_weight', 'font_style', 'line_height', 'letter_spacing', 'text_transform', 'font_variant' );
	...
	public static function createFontScript() {
		...
		jQuery(document).ready(function($) {
			...
			var _sr_select_font_throttle = null;
			...
			$('.tf-font .tf-font-sel-color, .tf-font .tf-font-sel-shadow-color').wpColorPicker({
				change: function ( event, ui ) {
					...
					if ( _sr_select_font_throttle != null ) {
						clearTimeout( _sr_select_font_throttle );
						_sr_select_font_throttle = null;
					}
					var $this = $(this);
					_sr_select_font_throttle = setTimeout( function() {
						sr_select_font_update_preview( $this.parents('.tf-font:eq(0)'), true );
					}, 300 );
				}
			});
		...
		function sr_select_font_update_preview( $container, doTrigger ) {
			"use strict";
			var $ = jQuery;
			...
			<select class='tf-font-sel-family'>
				...
				if ( $this->settings['show_google_fonts'] ) {
					...
					<optgroup label="Google WebFonts" class='google'>
						<?php
						$allFonts = shapingrain_get_googlefonts();
	 ...
	 * @param	ShapingRainFrameworkCustomizerSection $section The section where this option will be placed
	 * @param	int                             $priority The order of this control in the section
	 * @return	void
	 * @since	1.4
	 */
	public function registerCustomizerControl( $wp_customize, $section, $priority = 1 ) {
		$wp_customize->add_control( new ShapingRainFrameworkOptionFontControl( $wp_customize, $this->getID(), array(
 ...
 */
add_action( 'customize_register', 'registerShapingRainFrameworkOptionFontControl', 1 );
 ...
 * @since	1.4
 */
function registerShapingRainFrameworkOptionFontControl() {
	class ShapingRainFrameworkOptionFontControl extends WP_Customize_Control {
		public $description;
		public $params;

		public function render_content() {
			$this->params['show_preview'] = false;
			ShapingRainFrameworkOptionFont::createFontScript();
			...
			$value = array_merge( ShapingRainFrameworkOptionFont::$defaultStyling, $value );
			...
			<label <?php echo $visibilityAttrs ?>>
				...
				<select class='tf-font-sel-family'>
				    ...
				    <optgroup label="Web Safe Fonts" class='safe'>
						<?php
						foreach ( ShapingRainFrameworkOptionFont::$webSafeFonts as $family => $label ) {
							printf( "<option value='%s'%s>%s</option>",
								$family,
								selected( $value['font-family'], $family, false ),
								$label
