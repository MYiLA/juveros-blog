...
class ShapingRainFrameworkOptionFile extends ShapingRainFrameworkOption {
    ...
    function __construct( $settings, $owner ) {
        ...
        add_filter( 'sr_generate_css_upload_' . $this->getOptionNamespace(), array( $this, 'generateCSS' ), 10, 2 );

        add_action( 'sr_livepreview_pre_' . $this->getOptionNamespace(), array( $this, 'preLivePreview' ), 10, 3 );
        add_action( 'sr_livepreview_post_' . $this->getOptionNamespace(), array( $this, 'postLivePreview' ), 10, 3 );
    }
     ...
     * @since 1.9
     *
     * @see sr_file_upload_option_customizer_get_value()
     */
    public function preLivePreview( $optionID, $optionType, $option ) {
        ...
        $nonce = wp_create_nonce( 'sr_file_upload_option_nonce' );

        ?>
        wp.ajax.send( 'sr_file_upload_option_customizer_get_value', {
        data: {
    ...
    public function registerCustomizerControl( $wp_customize, $section, $priority = 1 ) {
        $wp_customize->add_control( new ShapingRainFrameworkOptionFileUploadControl( $wp_customize, $this->getID(), array(
 ...
 */
add_action( 'customize_register', 'registerShapingRainFrameworkOptionFileUploadControl', 1 );
function registerShapingRainFrameworkOptionFileUploadControl() {
    class ShapingRainFrameworkOptionFileUploadControl extends WP_Customize_Control {
        public $description;
        public $caption;
        
        public function render_content() {
            ShapingRainFrameworkOptionFile::createUploaderScript();
...
if ( ! function_exists( 'sr_file_upload_option_customizer_get_value' ) ) {

    add_action( 'wp_ajax_sr_file_upload_option_customizer_get_value', 'sr_file_upload_option_customizer_get_value' );

    /**
     * Returns the image URL from an attachment ID & size.
     *
     * @see ShapingRainFrameworkOptionUpload->preLivePreview().
     */
    function sr_file_upload_option_customizer_get_value() {
        ...
        if ( ! empty( $_POST['nonce'] ) && ! empty( $_POST['id'] ) ) {
            ...
            if ( wp_verify_nonce( $nonce, 'sr_file_upload_option_nonce' ) ) {
