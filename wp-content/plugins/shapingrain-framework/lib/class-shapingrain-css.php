...
/**
 * ShapingRain Framework CSS Class
 * In charge of creating and parsing CSS rules created from framework options.
 *
 * @author Benjamin Intal
 **/
class ShapingRainFrameworkCSS {

	// Compression type to use
	const SCSS_COMPRESSION = 'shapingrainscss_formatter_compressed';
	 ...
	 * Class constructor
	 *
	 * @param   ShapingRainFramework $frameworkInstance an instance of the framework object
	 * @return  void
	 * @since   1.2
	 */
	function __construct( $frameworkInstance ) {
		...
		add_action( 'sr_create_option_' . $frameworkInstance->optionNamespace, array( $this, 'getOptionsWithCSS' ) );
		...
		add_action( 'sr_admin_options_saved_' . $frameworkInstance->optionNamespace, array( $this, 'generateSaveCSS' ) );
		// Trigger compile when there are no default options saved yet
		add_action( 'sr_init_no_options_' . $frameworkInstance->optionNamespace, array( $this, 'generateMissingCSS' ) );
	}
	...
	public function addAdminCSS( $cssString ) {
		$this->adminCSS[] = $cssString;
	}
	 ...
	 *
	 * @param   ShapingRainFrameworkOption $option The option which was just added
	 * @return  void
	 * @since   1.2
	 */
	...
	private function getCSSFilePath() {
		...
		return apply_filters( "sr_css_get_css_file_path_{$namespace}", $uploadsFolder . $this->getCSSSlug() . '.css' );
	}
	...
	public function generateCSS() {
		...
		foreach ( $this->allOptionsWithIDs as $option ) {
			...
			if ( ! apply_filters( 'sr_continue_generate_css_' . $option->settings['type'] . '_' . $option->getOptionNamespace(), true, $option ) ) {
				continue;
			}

			// Custom generated CSS.
			$generatedCSS = apply_filters( 'sr_generate_css_' . $option->settings['type'] . '_' . $option->getOptionNamespace(), '', $option );
		...
		// Add additional CSS added via ShapingRainFramework::createCSS()
	 ...
	 * the ShapingRainFramework->createCSS( '...' ) function.
	 *
	 * @return	void
	 * @since	1.3
	 */
