...
class ShapingRainFrameworkCustomizer {
	...
	function __construct( $settings, $owner ) {
		...
		sr_add_action_once( 'customize_controls_enqueue_scripts', array( $this, 'loadUploaderScript' ) );

		// Clear local storage, we use it for remembering modified customizer values.
		sr_add_action_once( 'customize_controls_print_footer_scripts', array( $this, 'initLocalStorage' ) );

		// Generate the custom CSS for live previews.
		sr_add_action_once( 'wp_ajax_sr_generate_customizer_css', array( $this, 'ajaxGenerateCustomizerCSS' ) );

		// Modify the values of the options for the generation of CSS with the values from the customizer $_POST.
		global $wp_customize;
		if ( isset( $wp_customize ) ) {
			sr_add_filter_once( 'sr_pre_get_value_' . $this->owner->optionNamespace, array( $this, 'useCustomizerModifiedValue' ), 10, 3 );
		}
	}

	public function loadUploaderScript() {
		wp_enqueue_media();
		wp_enqueue_script( 'tf-theme-customizer-serialize', ShapingRainFramework::getURL( '../js/min/serialize-min.js', __FILE__ ) );
		wp_enqueue_style( 'tf-admin-theme-customizer-styles', ShapingRainFramework::getURL( '../css/admin-theme-customizer-styles.css', __FILE__ ) );
	}
	...
	public function ajaxGenerateCustomizerCSS() {
		...
		foreach ( ShapingRainFramework::getAllInstances() as $framework ) {

			// Modify the values of the options for the generation of CSS with the values from the customizer $_POST.
			$namespace = $framework->optionNamespace;
			add_filter( "sr_pre_get_value_{$namespace}", array( $this, 'useCustomizerModifiedValue' ), 10, 3 );
			...
			/**
			 * Allow options to add customizer live preview parameters. The sr_generate_customizer_preview_js hook allows for manipulating these values.
			 *
			 * @since 1.9.2
			 *
			 * @see sr_generate_customizer_preview_js
			 */
			$generated = apply_filters( "sr_generate_customizer_preview_css_{$namespace}", $generated );
	 ...
	 * @param ShapingRainFrameworkOption $option The option being parsed.
	 *
	 * @return mixed The new value
	 *
	 * @see sr_pre_get_value_{namespace}
	 */
	...
	public function livePreviewMainScript() {
		?>
		<script>
		window.sr_refresh_css = function() {
			if ( typeof localStorage !== 'undefined' ) {
				...
				wp.ajax.send( 'sr_generate_customizer_css', {
				    success: function( data ) {

						// Add the modified CSS ShapingRain has generated from the preview values.
						var style = document.querySelector('style#sr_live_preview');
						if ( ! style ) {
							var style = document.createElement('STYLE');
							style.setAttribute( 'id', 'sr_live_preview' );
						 ...
						 */
						do_action( 'sr_generate_customizer_preview_js' );
						?>
				    },
				...
				v.bind( function( value ) {
					...
					if ( empty( $option->settings['livepreview'] ) ) {
						...
						if ( ! empty( $option->settings['id'] ) ) {
							do_action( 'sr_livepreview_pre_' . $this->owner->optionNamespace, $option->settings['id'], $option->settings['type'], $option );
						}
						...
						if ( ! empty( $option->settings['id'] ) ) {
							do_action( 'sr_livepreview_post_' . $this->owner->optionNamespace, $option->settings['id'], $option->settings['type'], $option );
						}
					}
	...
	public function printPreviewCSS() {
		...
		if ( ! in_array( $this->owner->optionNamespace, self::$namespacesWithPrintedPreviewCSS ) ) {
			...
			echo '<style id="shapingrain-preview-' . esc_attr( $this->owner->optionNamespace ) . '">';
	...
	public function register( $wp_customize ) {
		...
		foreach ( $this->options as $index => $option ) {
			if ( ! empty( $option->settings['id'] ) ) {
				 ...
				 * @since 1.9.2
				 */
				$transport = apply_filters( "sr_customizer_transport_{$option_type}_{$namespace}", $transport, $option );
		...
		sr_add_action_once( 'wp_footer', array( $this, 'livePreviewMainScript' ) );
	}

	public function createOption( $settings ) {
		if ( ! apply_filters( 'sr_create_option_continue_' . $this->owner->optionNamespace, true, $settings ) ) {
			return null;
		}
		...
		do_action( 'sr_create_option_' . $this->owner->optionNamespace, $obj );
