...
if ( class_exists( 'ShapingRainFrameworkOption' ) ) {
	...
	class ShapingRainFrameworkOptionEddLicense extends ShapingRainFrameworkOption {
		...
		function __construct( $settings, $owner ) {
			...
			add_action( 'sr_create_option_' . $this->getOptionNamespace(), array( $this, 'activateLicense' ) );
		}
		...
		public function activateLicense( $option ) {
			...
			if ( strlen( $license ) > 0 ) {
				...
				if ( false == get_transient( "sr_edd_license_try_$key" ) ) {
					$this->check( $license, 'activate_license' );
				}
			}
		...
		public function display() {
			...
			if ( strlen( $license ) > 0 ) {
				...
				$status = get_transient( "sr_edd_license_status_$key" );
		...
		public function registerCustomizerControl( $wp_customize, $section, $priority = 1 ) {
			 ...
			 */
			$wp_customize->add_control( new ShapingRainFrameworkCustomizeControl( $wp_customize, $this->getID(), array(
		...
		public function check( $license = '', $action = 'check_license' ) {
			...
			$status_lifetime     = apply_filters( 'sr_edd_license_status_lifetime', 48 * 60 * 60 );         // Default is set to two days
			$activation_lifetime = apply_filters( 'sr_edd_license_activation_lifetime', 365 * 24 * 60 * 60 ); // Default is set to one year
			...
			if ( 'activate_license' == $action ) {
				...
				if ( 'invalid' == $license_data->license ) {
					set_transient( "sr_edd_license_status_$key", 'invalid', $status_lifetime );
					set_transient( "sr_edd_license_try_$key", true, $activation_lifetime );
					return 'invalid';
				}
				 ...
				 *
				 * @link https://github.com/gambitph/ShapingRain-Framework/issues/203
				 */
				$status = $this->check( $license );
				...
				set_transient( "sr_edd_license_status_$key", $license_data->license, $status_lifetime );
		...
		public function checkUpdates() {
			...
			if ( 'plugin' == $item_is ) {
				...
				if ( ! class_exists( 'SHAPINGRAIN_EDD_SL_Plugin_Updater' ) ) {
			...
			elseif ( in_array( $item_is, array( 'theme-parent', 'theme-child' ) ) ) {
				...
				if ( ! class_exists( 'SHAPINGRAIN_EDD_Theme_Updater' ) ) {
			...
			/* Setup updater */
			if ( 'plugin' == $item_is ) {
				$edd_updater = new SHAPINGRAIN_EDD_SL_Plugin_Updater( $endpoint, $this->settings['file'], $args );
			} else {
				new SHAPINGRAIN_EDD_Theme_Updater( $args, $strings );
			}
		...
		public function item_is( $file ) {
			 ...
			 * correct URL output.
			 *
			 * @see  ShapingRainFramework::getURL()
			 */
