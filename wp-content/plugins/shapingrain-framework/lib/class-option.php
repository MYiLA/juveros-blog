...
class ShapingRainFrameworkOption {
	...
	private static $defaultSettings = array(
		 ...
		 * @since 1.0
		 * @var string
		 * @see http://www.shapingrainframework.net/livepreview-parameter
		 */
		 ...
		 * @since 1.0
		 * @var string
		 * @see http://www.shapingrainframework.net/generate-css-automatically-for-your-options/
		 */
		'css' => '',
	...
	public static function factory( $settings, $owner ) {
		...
		$className = 'ShapingRainFrameworkOption' . str_replace( ' ', '', ucwords( str_replace( '-', ' ', $settings['type'] ) ) );

		// assume all the classes are already required
		if ( ! class_exists( $className ) && ! class_exists( $settings['type'] ) ) {
			ShapingRainFramework::displayFrameworkError(
	...
	function __construct( $settings, $owner ) {
		...
		$this->type = is_a( $owner, 'ShapingRainFrameworkMetaBox' ) ? self::TYPE_META : self::TYPE_ADMIN;
		$this->type = is_a( $owner, 'ShapingRainFrameworkCustomizer' ) ? self::TYPE_CUSTOMIZER : $this->type;
	...
	public function getValue( $postID = null ) {
		 ...
		 * @since 1.9.2
		 */
		$value = apply_filters( 'sr_pre_get_value_' . $this->getOptionNamespace(), $value, $postID, $this );
		 ...
		 * @since 1.9
		 */
		return apply_filters( 'sr_get_value_' . $this->settings['type'] . '_' . $this->getOptionNamespace(), $value, $postID, $this );
	}
	...
	public function setValue( $value, $postID = null ) {
		...
		do_action( 'sr_set_value_' . $this->settings['type'] . '_' . $this->getOptionNamespace(), $value, $postID, $this );
	...
	protected function getFramework() {
		if ( is_a( $this->owner, 'ShapingRainFrameworkAdminTab' ) ) {
	...
	protected function echoOptionHeader( $showDesc = false ) {
		...
		$useCustom = false;
		$useCustom = apply_filters( 'sr_use_custom_option_header', $useCustom );
		$useCustom = apply_filters( 'sr_use_custom_option_header_' . $this->getOptionNamespace(), $useCustom );
		if ( $useCustom ) {
			do_action( 'sr_custom_option_header', $this );
			do_action( 'sr_custom_option_header_' . $this->getOptionNamespace(), $this );
			return;
		}
		...
		?>
		<tr valign="top" class="<?php if ( ! empty ($class ) ) echo $class . ' '; ?>row-<?php echo self::$rowIndex ?> <?php echo $evenOdd ?>" <?php echo $style ?>>
	...
	protected function echoOptionFooter( $showDesc = true ) {
		...
		// Allow overriding for custom styling
		$useCustom = false;
		$useCustom = apply_filters( 'sr_use_custom_option_footer', $useCustom );
		$useCustom = apply_filters( 'sr_use_custom_option_footer_' . $this->getOptionNamespace(), $useCustom );
		if ( $useCustom ) {
			do_action( 'sr_custom_option_footer', $this );
			do_action( 'sr_custom_option_footer_' . $this->getOptionNamespace(), $this );
			return;
		}
