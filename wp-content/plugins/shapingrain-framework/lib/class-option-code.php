 ...
 * @package ShapingRain Framework Core
 **/
 ...
 * @since	1.3
 **/
class ShapingRainFrameworkOptionCode extends ShapingRainFrameworkOption {
	...
	function __construct( $settings, $owner ) {
		...
		add_filter( 'sr_generate_css_code_' . $this->getOptionNamespace(), array( $this, 'generateCSSCode' ), 10, 2 );
	...
	public function printJS() {
		...
		if ( ShapingRainFrameworkOption::TYPE_META == $this->type ) {
			return;
		}
	 ...
	 * ShapingRainFrameworkCSS, using sr_generate_css_code
	 *
	 * @param	string               $css The CSS to output
	 * @param	ShapingRainFrameworkOption $option The option object being generated
	 * @return	void
	 * @since	1.3
	 */
	public function generateCSSCode( $css, $option ) {
		...
		if ( ShapingRainFrameworkOption::TYPE_META != $option->type ) {
			$css = $this->getValue();
		}
		return $css;
	}
	...
	public function loadAdminScripts() {
		wp_enqueue_script( 'tf-ace', ShapingRainFramework::getURL( '../js/ace-min-noconflict/ace.js', __FILE__ ) );
		wp_enqueue_script(
			'tf-ace-theme-' . $this->settings['theme'],
			ShapingRainFramework::getURL( '../js/ace-min-noconflict/theme-' . $this->settings['theme'] . '.js',
			__FILE__ )
		);
		wp_enqueue_script(
			'tf-ace-mode-' . $this->settings['lang'],
			ShapingRainFramework::getURL( '../js/ace-min-noconflict/mode-' . $this->settings['lang'] . '.js',
			__FILE__ )
		);
	}
	...
	public function registerCustomizerControl( $wp_customize, $section, $priority = 1 ) {
		$wp_customize->add_control( new ShapingRainFrameworkOptionCodeControl( $wp_customize, $this->getID(), array(
 ...
 */
add_action( 'customize_register', 'registerShapingRainFrameworkOptionCodeControl', 1 );
 ...
 * @since	1.3
 */
function registerShapingRainFrameworkOptionCodeControl() {
	class ShapingRainFrameworkOptionCodeControl extends WP_Customize_Control {
		public $description;
		public $lang;
		public $theme;
		public $height;
