...
if ( ! function_exists( 'sr_hex2rgb' ) ) {
	function sr_hex2rgb( $hex ) {
		$hex = str_replace( '#', '', $hex );
 ...
 */
function sr_add_action_once( $tag, $function_to_add, $priority = 10, $accepted_args = 1 ) {
 ...
 * @return true
 */
function sr_add_filter_once( $tag, $function_to_add, $priority = 10, $accepted_args = 1 ) {
	global $_gambitFiltersRan;
 ...
 */
function sr_get_post_types( $public = true, $value = 'all' ) {
...
function sr_sideload_image( $file, $desc = null, $post_data = array() ) {
	$data = new stdClass();
	...
	if ( ! empty( $file ) ) {
		...
		$id = media_handle_sideload( $file_array, 0, $desc, $post_data );
		...
		// Update alt tag
		if ( ! empty ( $post_data['_wp_attachment_image_alt'] ) ) {
			update_post_meta( $id, '_wp_attachment_image_alt', $post_data['_wp_attachment_image_alt'] );
		}
