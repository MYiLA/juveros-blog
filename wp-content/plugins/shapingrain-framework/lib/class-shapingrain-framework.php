...
class ShapingRainFramework {
	...
	/**
	 * All ShapingRainFramework instances
	 *
	 * @var array
	 */
  ...
  private $blogId;
	 ...
	 *
	 * @var array of ShapingRainFrameworkAdminPage, ShapingRainFrameworkMetaBox, & ShapingRainFrameworkCustomizer
	 */
	private $mainContainers = array();

	/**
	 * All Google Font options used. This is for enqueuing Google Fonts for the frontend
	 * TODO Move this to the ShapingRainFrameworkOptionSelectGooglefont class and let it enqueue from there
	 *
	 * @var array ShapingRainFrameworkOptionSelectGooglefont
	 */
	 ...
	 *
	 * @var array of ShapingRainFrameworkOption
	 */
	...
	private $defaultSettings = array(
		'css' => 'inline', 	// If 'generate', ShapingRain will try and generate a cacheable
	...
	/**
	 * Gets all active instances of ShapingRain Framework
	 *
	 * @since 1.9.2
	 *
	 * @return array An array of ShapingRainFramework objects
	 */
	...
	/**
	 * Creates a new ShapingRainFramework object
	 *
	 * @since 1.0
	 *
	 * @param string $optionNamespace The namespace to get options from.
    ...
    $this->blogId = get_current_blog_id();
		...
		do_action( 'sr_init_' . $this->optionNamespace, $this );

		$this->cssInstance = new ShapingRainFrameworkCSS( $this );

		add_action( 'admin_enqueue_scripts', array( $this, 'loadAdminScripts' ) );
		add_action( 'sr_create_option_' . $this->optionNamespace, array( $this, 'rememberAllOptions' ) );
		add_filter( 'sr_create_option_continue_' . $this->optionNamespace, array( $this, 'removeChildThemeOptions' ), 10, 2 );
	...
	/**
	 * Action hook on sr_create_option to remember all the options, used to ensure that our
	 * serialized option does not get cluttered with unused options
	 *
	 * @since 1.2.1
	 *
	 * @param ShapingRainFrameworkOption $option The option that was just created.
	 *
	 * @return void
	 */
	...
	/**
	 * Loads all the admin scripts used by ShapingRain Framework
	 *
	 * @since 1.0
	 *
	...
	public function loadAdminScripts( $hook ) {
		...
		// Only enqueue scripts if we're on a ShapingRain options page.
		if ( in_array( $hook, $panel_ids ) || ! empty( $this->mainContainers['meta-box'] ) ) {
			wp_enqueue_media();
			wp_enqueue_script( 'tf-serialize', ShapingRainFramework::getURL( '../js/min/serialize-min.js', __FILE__ ) );
			wp_enqueue_script( 'tf-styling', ShapingRainFramework::getURL( '../js/min/admin-styling-min.js', __FILE__ ) );
			wp_enqueue_style( 'tf-admin-styles', ShapingRainFramework::getURL( '../css/admin-styles.css', __FILE__ ) );
		}
	}
    ...
    // Reload options if blog has been switched
		...
		if ( false === $currentOptions ) {
			do_action( 'sr_init_no_options_' . $this->optionNamespace );
		}
	 ...
	 *
	 * @see ShapingRainFrameworkOption->getValue()
	 */
	 ...
	 *
	 * @see ShapingRainFrameworkOption->setValue()
	 */
	...
	public function saveInternalAdminPageOptions() {
		...
		do_action( 'sr_save_options_' . $this->optionNamespace );
		return $this->adminOptions;
	}
	 ...
	 *
	 * @return ShapingRainFrameworkAdminPage The created admin page
	 */
   ...
   *
 	 ...
 	 *
	 *	@return ShapingRainFrameworkAdminPage The created sample coennt page.
	 */
	...
	public function createAdminPage( $settings ) {
		...
		do_action( 'sr_admin_panel_created_' . $this->optionNamespace, $container );
		return $container;
	}
	 ...
	 *
	 * @return ShapingRainFrameworkMetaBox The created meta box
	 */
	 ...
	 *
	 * @return ShapingRainFrameworkCustomizer The created section
	 */
	...
	public function createCustomizer( $settings ) {
		...
		$container = $this->createContainer( $settings );
		do_action( 'sr_theme_customizer_created_' . $this->optionNamespace, $container );
		return $container;
	}
	 ...
	 *
	 * @return ShapingRainFrameworkCustomizer|ShapingRainFrameworkAdminPage|ShapingRainFrameworkMetaBox The created container
	 */
	public function createContainer( $settings ) {
		...
		$class = 'ShapingRainFramework' . str_replace( ' ', '', ucfirst( str_replace( '-', ' ', $settings['type'] ) ) );
		...
		do_action( 'sr_' . $action . '_created_' . $this->optionNamespace, $container );
	...
	/**
	 * Hook to the sr_create_option_continue filter, to check whether or not to continue
	 * adding an option (if the option id was used in $shapingrain->removeOption).
	 *
	 * @since 1.2.1
	 * @access public
	 *
	...
	public function getOption( $optionName, $postID = null ) {
		...
		return apply_filters( 'sr_get_option_' . $this->optionNamespace, $value, $optionName, $postID );
	}
	...
	public function getOptions( $optionArray, $postID = null ) {
		...
		return apply_filters( 'sr_get_options_' . $this->optionNamespace, $optionArray, $postID );
	}
	...
	public function setOption( $optionName, $value, $postID = null ) {
		...
		do_action( 'sr_set_option_' . $this->optionNamespace, $optionName, $value, $postID );
	...
	public function createAdminCSS ( $CSSString ) {
		$this->cssInstance->addAdminCSS( $CSSString );
	}
	...
	public function set( $setting, $value ) {
		...
		do_action( 'sr_setting_' . $setting . '_changed_' . $this->optionNamespace, $value, $oldValue );
	}
	...
	public function generateAdminCSS() {
		return $this->cssInstance->generateAdminCSS();
	}


	/**
	 * Adds a 'sr_save_option_{namespace}_{optionID}' filter to all Customizer options
	 * which are just about to be saved
	 *
	 * This uses the `pre_update_option` filter to check all the options being saved if it's
	 * a theme_mod option. It further checks whether these are ShapingRain customizer options,
	...
	public function addCustomizerSaveFilter( $value, $optionName, $oldValue ) {
		...
		// Checks whether a ShapingRain customizer is in place.
			...
			foreach ( $this->mainContainers['customizer'] as $customizer ) {
				foreach ( $customizer->options as $option ) {
					if ( ! empty( $option->settings['id'] ) ) {
						...
						// Hook 'sr_save_option_{namespace}'.
						$newValue = apply_filters( 'sr_save_option_' . $this->optionNamespace, $tempValue, $option->settings['id'] );

						// Hook 'sr_save_option_{namespace}_{optionID}'.
						$newValue = apply_filters( 'sr_save_option_' . $themeModName, $tempValue );
		...
		if ( $customizerUsed ) {
			...
			do_action( "sr_pre_save_options_{$namespace}", $this->mainContainers['customizer'] );
		}
