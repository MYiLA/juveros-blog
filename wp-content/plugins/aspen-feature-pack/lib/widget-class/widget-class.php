 ...
 *
 * Modifications by ShapingRain.com Labs for ShapingRain.com themes
 * Copyright 2013-2019 by ShapingRain.com | www.shapingrain.com
 *
 *
	...
	class SR_Widget extends WP_Widget
	{
		...
		function form( $instance ) {
			...
			if (!empty($this->groups)) {
				...
				echo apply_filters( 'aspen_tabs_panel_group', $groups );
				echo '<div class="tabs-panel">';
			}

			$form = $this->create_fields();

			echo apply_filters ( 'aspen_tabs_panel_output', $form );
		...
		function create_field_posts( $key, $out = "" )
		{
			...
			if ($posts) {
				...
				$out .= '<option value="0"' . selected($key['value'], 0, false) . '>'. esc_html__( 'None', 'aspen' ) . '</option>';
				foreach ($posts as $p) {
		...
		function create_field_categories( $key, $out = "" )
		{
			...
			if ($categories) {
				...
				if (! $key['multiple']) {
					...
					$out .= '<option value="0"' . selected($key['value'], 0, false) . '>'. esc_html__( 'All', 'aspen' ) . '</option>';
		...
		function create_field_slider( $key, $out = "" )
		{
			...
			$out .= '<small class="description">'. esc_html__('Select control and user interaction options.', 'aspen' ) . '</small><br/>';
			...
			if  (!in_array('controls', $key['hidden'] ) ) {
				...
				$out .=	'/> ';
				$out .=  ' ' . esc_html__('Display navigation arrows', 'aspen') . '</label>';
				$out .= '<br/>';
			}
			...
			if  (!in_array('pips', $key['hidden'] ) ) {
				...
				$out .= checked( $meta , true, false );
				$out .= ' /> ';
				$out .=  ' ' . esc_html__('Display item navigation pips', 'aspen') . '</label>';
			...
			if  (!in_array('pauseonhover', $key['hidden'] ) ) {
				...
				$out .= checked( $meta , true, false );
				$out .= ' /> ';
				$out .=  ' ' . esc_html__('Pause when the user hovers over the slider', 'aspen') . '</label>';
			...
			if  (!in_array('pauseonaction', $key['hidden'] ) ) {
				...
				$out .= checked( $meta , true, false );
				$out .= ' /> ';
				$out .=  ' ' . esc_html__('Pause when the user interacts with control elements', 'aspen') . '</label>';
			...
			if  (!in_array('randomize', $key['hidden'] ) ) {
				...
				$out .= checked( $meta , true, false );
				$out .= ' /> ';
				$out .=  ' ' . esc_html__('Randomize slide order', 'aspen') . '</label>';
			...
			if  ( !in_array('transition', $key['hidden']) && !in_array('speed', $key['hidden']) ) {
				$out .= '<br/><small class="description">' . esc_html__('Select which transition to use and set the speed of the slideshow cycling in milliseconds.', 'aspen' ) . '</small><br/>';
			}
			elseif ( in_array('transition', $key['hidden'])  ) {
				$out .= '<br/><small class="description">' . esc_html__('Set the speed of the slideshow cycling in milliseconds.', 'aspen' ) . '</small><br/>';
			} else {
				$out .= '<br/><small class="description">'. esc_html__( 'Select which transition to use.', 'aspen' ) . '</small><br/>';
			}
			...
			if  (!in_array('transition', $key['hidden'] ) ) {
				...
				$options = array(
						array( 'value' => 'slide', 'name' => esc_html__('Slide', 'aspen') ),
						array( 'value' => 'fade', 'name'  => esc_html__('Fade', 'aspen') ),
				);
		...
		function create_field_gallery( $key, $out = "" )
		{
			...
			$out .= '<input type="button" class="button gallery-picker-select" data-target="#' . esc_attr( $key['_id'] ) . '" id="'. esc_attr( $key['_id'] ) . '_ids' .'" value="'.esc_html__('Select Items', 'aspen').'">';
		...
		function create_field_image( $key, $out = "" )
		{
			...
			if ( isset ($preview_url ) && $preview_url ) {
				$out .= '<div class="inbound-widget-preview"><img class="inbound-widget-preview-image" id="preview-' . esc_attr( $key['_id'] ) . '" src="' . esc_url( $preview_url ) . '"></div>';
			} else {
				$out .= '<div class="inbound-widget-preview"><img class="inbound-widget-preview-image empty" id="preview-' . esc_attr( $key['_id'] ) . '" src="' . get_template_directory_uri() . '/assets/images/admin/transparent.png"></div>';
			}
			...
			$out .= 'id="input-' . esc_attr( $key['_id'] ) . '" name="' . esc_attr( $key['_name'] ) . '" value="' . esc_attr ( $value ) . '" ';
			...
			$out .= '<input type="button" class="button select-img select-img-widget" data-target="' . esc_attr( $key['_id'] ) . '" value="' . esc_html__('Select Image', 'aspen') . '" />';
			$out .= '<input type="button" class="button select-img clear-img-widget" data-target="' . esc_attr( $key['_id'] ) . '" data-preview="' . get_template_directory_uri() . '/lib/admin/images/transparent.png" value="' . esc_html__('Clear', 'aspen') . '" />';
...
function sr_widget_scripts()
{
	...
	$plugin_meta = get_plugin_data( __FILE__, false, false );
	...
	wp_register_style( 'aspen-features-widgets-css', ASPEN_FEATURES_PLUGIN_URL . 'assets/css/widgets.css', false, $plugin_version );
	wp_enqueue_style( 'aspen-features-widgets-css' );
