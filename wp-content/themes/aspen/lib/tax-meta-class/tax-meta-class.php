...
/**
 * Tax Meta Class
 * Original by Ohad Raz (admin@bainternet.info), http://en.bainternet.info
 * Modified for ShapingRain
 *
...
if ( ! class_exists( 'SR_Tax_Meta_Class' ) ) :
     ...
     */
        ...
        <img src="';
					...
					echo '" alt="' . esc_attr__( 'Remove', 'aspen' ) . '" title="' . esc_attr__( 'Remove', 'aspen' ) . '" id="remove-' . $field['id'] . '"></div>';
					$c = $c + 1;
			...
			echo '" alt="' . __( 'Add', 'aspen' ) . '" title="' . __( 'Add', 'aspen' ) . '" id="add-' . $field['id'] . '"><br/></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
		...
		public function show_field_file( $field, $meta ) {
			...
			if ( $has_file ) {
				echo "<input type='button' class='{$multiple} button simplePanelfileUploadclear' id='{$id}' value='". esc_attr__('Remove File', 'aspen') . " ' data-mime_type='{$type}' data-ext='{$ext}'/>";
			} else {
				echo "<input type='button' class='{$multiple} button simplePanelfileUpload' id='{$id}' value='" . esc_attr__('Upload File', 'aspen') . "' data-mime_type='{$type}' data-ext='{$ext}'/>";
			}
			$this->show_field_end( $field, $meta );
		}
		...
		public function show_field_image( $field, $meta ) {
			...
			$w            = ! empty ( $field['width'] ) ? $field['width'] : 'auto';
			$h            = ! empty ( $field['height'] ) ? $field['height'] : 'auto';
			...
			echo "<input type='hidden' name='{$name}[url]' value='{$value['url']}'/>";
			if ( $has_image ) {
				echo "<input class='{$multiple} button  simplePanelimageUploadclear' id='{$id}' value='" . esc_html__( 'Remove Image', 'aspen' ) . "' type='button'/>";
			} else {
				echo "<input class='{$multiple} button simplePanelimageUpload' id='{$id}' value='" . esc_html__( 'Upload Image', 'aspen' ) . "' type='button'/>";
			}
			$this->show_field_end( $field, $meta );
		}
